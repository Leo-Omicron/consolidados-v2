  ...
  return (
    <div className="app-surface max-h-[600px] overflow-auto shadow-sm rounded-lg border app-border transition-premium">
      <div className="sticky top-0 z-10 app-surface-muted backdrop-blur-md px-4 py-3 border-b app-border flex font-semibold text-sm app-text transition-premium shadow-sm">
        <div className="w-1/3 cursor-pointer select-none" onClick={() => onSort('estudiante')}>
          Estudiante {getSortIcon('estudiante')}
        </div>
        <div className="flex-1 text-center">{viewMode === 'area' ? 'Áreas' : 'Asignaturas'}</div>
        <div className="w-24 text-right cursor-pointer select-none" onClick={() => onSort('aggregates.promActual')}>
                  ...
                  <table className="min-w-full divide-y app-divide text-sm">
                    <thead className="sticky top-0 z-10 app-surface-muted backdrop-blur-md border-b app-border">
                      <tr className="app-text-muted text-left">
                        ...
                        <th className="font-semibold pb-2 w-1/12 text-center cursor-pointer select-none" onClick={() => onSort(viewMode === 'area' ? 'defP1' : 'p1')}>
                          P1 {getSortIcon(viewMode === 'area' ? 'defP1' : 'p1')}
                        </th>
                        <th className="font-semibold pb-2 w-1/12 text-center cursor-pointer select-none" onClick={() => onSort(viewMode === 'area' ? 'defP2' : 'p2')}>
                          P2 {getSortIcon(viewMode === 'area' ? 'defP2' : 'p2')}
                        </th>
                        <th className="font-semibold pb-2 w-1/12 text-center cursor-pointer select-none" onClick={() => onSort(viewMode === 'area' ? 'defP3' : 'p3')}>
                        ...
                        <th className="font-semibold pb-2 w-1/12 text-center cursor-pointer select-none" onClick={() => onSort('tendencia')}>
                          Tendencia {getSortIcon('tendencia')}
                        </th>
                        <th className="font-semibold pb-2 w-1/12 text-center cursor-pointer select-none" onClick={() => onSort('promActual')}>
                          Prom.Calc {getSortIcon('promActual')}
                        </th>
                        <th className="font-semibold pb-2 w-1/12 text-center cursor-pointer select-none" onClick={() => onSort('p4Min')}>
                          ...
                          <React.Fragment key={idx}>
                            <tr className="app-surface-hover">
                              ...
                              <td className="py-2 text-center font-bold app-text">
                                {viewMode === 'area' ? rowAny.defA?.toFixed(2) ?? '-' : rowAny.a?.toFixed(2) ?? '-'}
                              </td>
                              <td className="py-2 text-center text-xl" title={`Tendencia: ${rowAny.tendencia}`}>
                                {rowAny.tendencia === 'up' ? '↗️' : rowAny.tendencia === 'down' ? '↘️' : rowAny.tendencia === 'flat' ? '➡️' : '-'}
                              </td>
                              <td className="py-2 text-center font-semibold app-text">
                                <GoalSeekCell
                                  rowId={row.id}
                                  currentValue={rowAny.promActual}
                                  notas={viewMode === 'area' ? { P1: rowAny.defP1 ?? null, P2: rowAny.defP2 ?? null, P3: rowAny.defP3 ?? null, P4: rowAny.defP4 ?? null } : { P1: rowAny.p1 ?? null, P2: rowAny.p2 ?? null, P3: rowAny.p3 ?? null, P4: rowAny.p4 ?? null }}
                                  config={config}
                                  evaluated={evaluated}
                                  hasP4={hasP4}
                              ...
                              <td className="py-2 text-center app-text-muted">
                                {rowAny.p4Min !== null && rowAny.p4Min !== undefined && rowAny.p4Min <= 5.0 ? (
                                  <span
                                    ...
                                    onClick={() => onSetSimulation(row.id, hasP4 ? 'P4' : 'P3', rowAny.p4Min!)}
                                  >
                              ...
                              <td className="py-2 text-center">
                                <StatusBadge text={rowAny.estado.text} color={rowAny.estado.color} />
                                {rowAny.p4Min !== null && rowAny.p4Min !== undefined && rowAny.p4Min > 5.0 && (
                                  <span
                                              ...
                                              <td className="p-2 text-center app-text-muted">
                                                {sub.p4Min !== null && sub.p4Min !== undefined && sub.p4Min <= 5.0 ? (
                                                  <span
                                                    className="cursor-pointer hover:text-amber-500 transition-colors border-b border-dashed border-amber-300 dark:border-amber-700"
                                                    title="Click para auto-completar nota mínima aprobatoria"
                                                    onClick={() => onSetSimulation(sub.id, hasP4 ? 'P4' : 'P3', sub.p4Min!)}
                                                  >
                                                    {sub.p4Min.toFixed(2)}
                                                  </span>
                                                ) : '-'}
