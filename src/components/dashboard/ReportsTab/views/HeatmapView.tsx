...
import type { HeatmapReport } from '../../../../domain/types';

export const HeatmapView: React.FC<{ data: HeatmapReport | null }> = ({ data }) => {
          ...
          <tbody className="bg-white divide-y divide-slate-100">
            {heatmapData.rows.map(row => (
              <tr key={row.studentName} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-3 py-2 text-center font-extrabold text-slate-900 bg-slate-50/40 print:w-[45px] print:min-w-0 print:text-[8px] print:px-1">{(row.promActual || 0).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
